...
def gradient_descent(problem, max_attempts=10, max_iters=np.inf,
    ...
    return best_state, best_fitness, fitness_curve, time_curve
    ...
    def fit(self, X, y=None, init_weights=None):
        ...
        if self.algorithm == 'random_hill_climb':
            ...
            for _ in range(self.restarts + 1):
                ...
                current_weights, current_loss, fitness_curve, time_curve = random_hill_climb(
                    problem,
            ...
            if init_weights is None:
                init_weights = np.random.uniform(-1, 1, num_nodes)
            fitted_weights, loss, fitness_curve, time_curve = simulated_annealing(
                problem,
            ...
            fitted_weights, loss, fitness_curve, time_curve = genetic_alg(
                problem,
            ...
            if init_weights is None:
                init_weights = np.random.uniform(-1, 1, num_nodes)
            fitted_weights, loss, fitness_curve, time_curve = gradient_descent(
                problem,
                max_attempts=self.max_attempts if self.early_stopping else
        ...
        return fitness_curve, time_curve
